  ...
  if (not line.strip().startswith('!')):
    ...
    if (line.startswith('tridiag_solve_method')):
      ...
      if ('modified_BVF_Ri_limiter' in parameters):
        if (not any('l_modify_limiters_for_cnvg_test' in tmpstr for tmpstr in input_lines)):
          lnew = 'l_modify_limiters_for_cnvg_test = .false.,' + '\n'
          line_collections.append(lnew)
          print('append line '+lnew)
      if ('modified_wp3_clip' in parameters):
        if (not any('l_use_wp3_lim_with_smth_Heaviside' in tmpstr for tmpstr in input_lines)):
          lnew = 'l_use_wp3_lim_with_smth_Heaviside = .false.,' + '\n'
          line_collections.append(lnew)
          print('append line '+lnew)
# "MOD_MODEL" file
...
model_file_name += '.in'
model_file_name = model_file_name.replace('turn-off-microphysics_turn-off-radiation_turn-off-splat_standard-aterms.in','default.in')
model_file_name = model_file_name.replace('turn-off-microphysics_turn-off-radiation_modified-bc_modified-ic_turn-off-splat_standard-aterms.in','baseline.in')
model_file_name = model_file_name.replace('turn-off-microphysics_turn-off-radiation_modified-bc_modified-ic_turn-off-splat_standard-aterms_smoothed-tau_linear-diffusion_modified-BVF-Ri-limiter_modified-wp3-clip.in','revall.in')
...
if ('refine' in parameters):
  ...
  config_strings['zt_grid_fname'] = f"'{grid_dir}/{grid_filename}'"
  shutil.move(grid_filename, os.path.join(grid_dir,grid_filename))
...
if os.path.isfile(model_file_name):
    ...
    print("Error: %s file not found" % model_file_name)

## Insert to model.in file
svalue = fnmatch.filter(caseFile_lines,'&model_setting*')
evalue = fnmatch.filter(caseFile_lines,'&statsnl*')
sind = caseFile_lines.index(svalue[0])
eind = caseFile_lines.index(evalue[0])
...
svalue = fnmatch.filter(caseFile_lines,'&clubb_params_nl*')
evalue = fnmatch.filter(caseFile_lines,'&silhs_params_nl*')
sind = caseFile_lines.index(svalue[0])
eind = caseFile_lines.index(evalue[0])
data = ''.join(caseFile_lines[sind:eind])
with open(os.path.join(tunable_dir,'tunable_parameters.in.template'),'w') as file:
    file.write(data)
print('Template tunable parameters file:',os.path.join(tunable_dir,'tunable_parameters.in.template'))

## Insert to configurable_model_flags.in file
svalue = fnmatch.filter(caseFile_lines,'&configurable_clubb_flags_nl*')
evalue = fnmatch.filter(caseFile_lines,'&model_setting*')
sind = caseFile_lines.index(svalue[0])
eind = caseFile_lines.index(evalue[0])
data = ''.join(caseFile_lines[sind:eind])
with open(os.path.join(tunable_dir,'configurable_model_flags.in.template'),'w') as file:
    file.write(data)
print('Template configurable flags file:',os.path.join(tunable_dir,'configurable_model_flags.in.template'))

## Insert to standard_stats.in file
